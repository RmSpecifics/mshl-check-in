(function () {
  ...
  var DEFAULT_SIGNOUT_WAIT_MINUTES = 10; // fallback if a session has no value set
  ...
  var currentSignoutWaitMinutes = DEFAULT_SIGNOUT_WAIT_MINUTES;
  ...
  function startSignOutCountdown() {
    ...
    function tick() {
      ...
      var waitMs = currentSignoutWaitMinutes * 60 * 1000;
  ...
  btnSubmit.addEventListener("click", function () {
    ...
    fetchSession().then(function (session) {
      ...
      return putSession(session).then(function (result) {
        ...
        currentSignoutWaitMinutes = (typeof session.signoutWaitMinutes === "number")
          ? session.signoutWaitMinutes
          : DEFAULT_SIGNOUT_WAIT_MINUTES;
  ...
  function renderAdmin() {
    ...
    fetchSession().then(function (session) {
      ...
      if (active) {
        ...
        var waitMins = (typeof session.signoutWaitMinutes === "number") ? session.signoutWaitMinutes : DEFAULT_SIGNOUT_WAIT_MINUTES;
        statusSub.textContent = "Created " + startedDate.toLocaleString("en-ZA", {
          day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit"
        }) + " — Expires " + expiresDate.toLocaleString("en-ZA", {
          day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit"
        }) + " — Sign-out after " + waitMins + " min";
  ...
  var signoutWaitMinutesInput = document.getElementById("signoutWaitMinutes");
  ...
  btnNewSession.addEventListener("click", function () {
    ...
    fetchSession().then(function (existing) {
      ...
      signoutWaitMinutesInput.value = String(DEFAULT_SIGNOUT_WAIT_MINUTES);
  ...
  btnConfirmDuration.addEventListener("click", function () {
    ...
    var signoutWaitMinutes = parseFloat(signoutWaitMinutesInput.value);
    if (isNaN(signoutWaitMinutes) || signoutWaitMinutes < 0) {
      durationPreview.innerHTML = '<span style="color:#B23A2E;">Enter a valid sign-out wait time (0 or more minutes).</span>';
      return;
    }
    ...
    var newSession = {
      ...
      durationHours: hours,
      signoutWaitMinutes: signoutWaitMinutes,
      entries: []
    };
